...
thet_each_sigma_point = sigma_points(3,:);

delta = [landmarkXs ; landmarkYs] - [xy_pose_each_sigma_point];

range = sum(delta .* delta,1);

bearing = normalize_angle(atan2(delta(2,:),delta(1,:)) - thet_each_sigma_point);
...
diff_xyth = sigma_points - mu;
diff_xyth(3,:) = normalize_angle(diff_xyth(3,:));
Sxz = wc .* diff_xyth * diff_z' ;
