...
for i = 1:m
	...
	if(observedLandmarks(landmarkId)==false)
		...
		mu_landmark = [mu(1); mu(2) ] + [ z(i).range * cos(normalize_angle(z(i).bearing + mu(3)));...
                                      z(i).range * sin(normalize_angle(z(i).bearing + mu(3)))];
    mu(2*landmarkId+2) = mu_landmark(1) ;
    mu(2*landmarkId+3) = mu_landmark(2) ;
  ...
  d = [mu(2*landmarkId+2); mu(2*landmarkId+3)] - [mu(1);mu(2)];
  q = (d')*d;
  zexp = [sqrt(q); normalize_angle(atan2(d(2),d(1)) - mu(3))];
  expectedZ(2*i-1:2*i) = zexp;
  
	% TODO: Compute the Jacobian Hi of the measurement function h for this observation
	
  low_Hi = (1/q)*[-sqrt(q)*d(1) , -sqrt(q)*d(2), 0, sqrt(q)*d(1), sqrt(q)*d(2); ... 
              d(2), -d(1), -q, -d(2), d(1)];
  Hi = zeros(2,3+2*N);
  Hi(:,1:3) = low_Hi(:,1:3);
  Hi(:,2*landmarkId+2:2*landmarkId+3) = low_Hi(:,4:5);
